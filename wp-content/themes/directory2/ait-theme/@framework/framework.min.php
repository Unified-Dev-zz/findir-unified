...
str_replace('%lang%',$langCode,$url);}}final
class
AitTheme
extends
NObject{private
...
AitUtils::startsWith($plugin['source'],'/plugins/')){self::$configuration['plugins'][$slug]['source']=aitPaths()->dir->ait.$plugin['source'];}add_theme_support($slug.'-plugin');}add_theme_support('ait-languages-plugin');}protected
static
function
tgmpa(){require_once
...
in_array($query['post_type'],$polylang->options["post_types"])){$query['lang']=AitLangs::getCurrentLanguageCode();}}return$query;}}abstract
class
AitFrontendAjax
extends
NObject{function
...
static
function
getFullCacheKey($cacheKey){global$wp_version;$cacheKey.=$wp_version.AIT_CURRENT_THEME.AIT_THEME_VERSION;return$cacheKey;}}