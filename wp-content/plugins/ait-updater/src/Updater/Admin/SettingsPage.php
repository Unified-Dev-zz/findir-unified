...
class SettingsPage
{
	...
	protected function processRequest()
	{
		...
		}elseif(isset($_GET['action']) and $_GET['action'] == 'fix-dirname'){

			check_admin_referer('actionfixdirname');

			$result = ThemeDirnameFixer::run();

			if(!empty($result)){
				set_transient('ait-updater-fixdir-msgs', $result, 30);
			}

			$goback = add_query_arg('dir-fixed', 'true',  remove_query_arg('action', wp_get_referer()));
			wp_redirect(esc_url_raw($goback));
			exit;
		}
	...
	public function render()
	{
		...
		<?php settings_errors() ?>
		<?php self::fixDirErrors() ?>

		<?php $result = Detector::checkIfActiveThemeIsRenamed(); ?>
		...
		<div class="notice notice-warning">
			<?php
				...
				if($result['type'] == 'parent'){
					printf(
						__('<p>We detected that you probably renamed folder name of the <strong>%1$s</strong> theme, which you are using as parent theme for the currently active theme. This will cause problems with parent theme updates. The current name of the theme folder is <code><strong>%2$s</strong></code> and has to be <code><strong>%3$s</strong></code>.</p><p><a href="%4$s" class="button button-primary">Fix it</a></p>', 'ait-updater'),
						$result['parent_theme_name'],
						$result['dir_name'],
						$result['codename'],
						$url
					);
					...
					printf(
						__('<p>We detected that you probably renamed folder name of the currently active <strong>%1$s</strong> theme and this will cause problems with theme updates. The current name of the theme folder is <code><strong>%2$s</strong></code> and has to be <code><strong>%3$s</strong></code>.</p><p><a href="%4$s" class="button button-primary">Fix it</a></p>', 'ait-updater'),
						$result['theme_name'],
						$result['dir_name'],
						$result['codename'],
						$url
	...
	protected function fixDirErrors()
	{
		if(isset($_GET['dir-fixed']) and ($msgs = get_transient('ait-updater-fixdir-msgs'))){
			$output = '';
			foreach($msgs as $msg){
				$c = $msg['is_error'] ? 'error' : 'success';
				$output .= "<div class='notice notice-{$c}'> \n";
				$output .= "<p><strong>{$msg['msg']}</strong></p>";
				$output .= "</div> \n";
			}
			echo $output;
			delete_transient('ait-updater-fixdir-msgs');
			remove_query_arg('dir-fixed', wp_get_referer());
		}
	}
	...
	public function printScripts()
	{
		...
		<script>
			jQuery(function($){
				...
				var showDefault = function($input){
					var $msg = $input.next('.ait-updater-error-msg');
					$input.removeClass('ait-field-error');
					$input.removeClass('ait-field-ok');
					$input.next('b').remove();
				...
				var checkKey = function($input){
					...
					if(id.indexOf('api_key') > -1){
						if(key.length > 0){
							if(id == 'api_key'){
								(key.length != 64) ? showError($input) : showOk($input);
							}
							if(id == 'envato_api_key'){
								(key.length != 32) ? showError($input) : showOk($input);
							}
						}else{
							showDefault($input);
						}
