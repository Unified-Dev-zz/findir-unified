;( function( $, angular )
{
	...
	app.controller('BuilderController', function( $scope, $window, focus )
	{
		...
		{
			var field = $scope.meta.fields[$index];
			var tabCount = 0;

			if (field.type === 'tab' && $index === 0) {
				angular.forEach($scope.meta.fields, function(field) {
					if (field.type === 'tab')
						tabCount++;
				});
	    ...
	    // Todo: Remove jQuery code below and use AngularJS
	   	$( '.field-edit-content .show-advanced' ).live( 'click', function()
	   	{
	   		$( this ).hide();
	   		$( this ).parent().find('.field-advanced').slideDown( 200 );
	   	} );

	   	$( '.field-edit-content .hide-advanced' ).live( 'click', function()
	   	{
