...
class Meta_Box_Builder
{
	...
	public function enqueue()
	{
		...
		wp_enqueue_style( 'meta-box-builder', MBB_CSS_URL . 'style.css', array(), '1.0.0' );
		...
		wp_register_script( 'angular-ui-sortable', MBB_JS_URL . 'sortable.js', array(), '1.3.2', true );
		wp_register_script( 'tg-dynamic-directive', MBB_JS_URL . 'tg.dynamic.directive.js', array(), '0.3.0', true );
		wp_register_script( 'meta-box-builder-directives', MBB_JS_URL . 'directives.js', array( 'angularjs' ), '2.0', true );
		wp_register_script( 'select2', RWMB_JS_URL . 'select2/select2.min.js', array( 'jquery' ), '3.2', true );
		...
		wp_enqueue_script( 'meta-box-builder', MBB_JS_URL . 'builder.js', array(
			'angularjs-animate', 'meta-box-builder-directives', 'select2',
	...
	public function register_meta_box( $meta_boxes )
	{
		...
		if ( ! empty( $availables ) )
		{
			foreach ( $availables as $meta_box )
			{
				$meta_boxes[] = @unserialize( $meta_box->post_content );
			}
		}
