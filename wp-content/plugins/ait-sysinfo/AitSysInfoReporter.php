...
class AitSysInfoReporter
{
	...
	protected $sysinfoPlugin;
	protected $columnWidth = 30;



	protected function __construct($sysinfoPlugin)
	{
		$this->sysinfoPlugin = $sysinfoPlugin;
	}
	...
	public function generateReport()
	{
		?>
		<p>
			<button class="button button-default" id="ait-sysinfo-copy">Copy to clipboard</button>
			<textarea readonly="readonly" wrap="off" id="ait-sysinfo-report"><?php echo $this->getReport(); ?></textarea>
		</p>
		<?php
	}
	...
	protected function getMemoryUsage()
	{
		...
		$percentage = round($value / ini_get('memory_limit') * 100, 0);
	...
	{
		$s = '';
		$aitTime = time() + (2 * HOUR_IN_SECONDS);
		$s .= gmdate('Y-m-d H:i:s', current_time('timestamp')) . " (server time)";
	...
	public static function getInstance($sysinfoPlugin)
	{
		if(!self::$instance){
			self::$instance = new self($sysinfoPlugin);
		}
